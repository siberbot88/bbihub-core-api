<div>
  <div class="flex items-center justify-between border-b px-4 py-3 text-sm">
    ...
    <div class="text-neutral-500">Total: {{ number_format($rows?->total() ?? 0) }} Pengguna</div>
  </div>
      ...
      <tbody class="divide-y divide-neutral-100">
        @forelse($rows as $u)
          <tr class="hover:bg-neutral-50">
            ...
            <td class="px-4 py-3">
              @php
                $active = isset($u->status) ? $u->status === 'active' : (bool) ($u->email_verified_at ?? false);
              @endphp
        ...
        @empty
          <tr><td colspan="6" class="px-4 py-6 text-center text-neutral-500">Tidak ada data.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
